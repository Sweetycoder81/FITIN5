...
const Memberships = () => {
  ...
  const handleSubscribe = (membershipId, e) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  const hasActiveMembership = () => {
    ...
    return user?.membership && user?.membershipExpiry && new Date(user.membershipExpiry) > new Date();
  };

  // Get user's current membership details
  const getCurrentMembership = () => {
    if (!hasActiveMembership() || !Array.isArray(memberships)) return null;
    return memberships.find(m => m?._id === user?.membership) || null;
  };
      ...
      {hasActiveMembership() && currentMembership ? (
        <div className="current-membership-alert mb-5">
          <div className="alert alert-success">
            ...
            <p>
              You are currently subscribed to the <strong>{currentMembership?.name || 'Unknown'}</strong> plan.
              {user?.membershipExpiry && (
                <span> Your membership expires on {new Date(user.membershipExpiry).toLocaleDateString()}.</span>
              )}
            </p>
          </div>
        </div>
              ...
              <Card className={`h-100 membership-card ${membership.isPopular ? 'popular-plan' : ''}`}>
                ...
                <Card.Footer className="bg-white border-0 text-center py-4">
                  <Button
                    ...
                    disabled={hasActiveMembership() && user.membership === membership._id}
                  >
                    {hasActiveMembership() && user.membership === membership._id
                      ? 'Current Plan'
                      : 'Subscribe Now'}
                  </Button>
                </Card.Footer>
