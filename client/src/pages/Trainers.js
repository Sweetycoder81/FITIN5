...
const Trainers = () => {
  ...
  const filteredTrainers = trainers.filter(
    (trainer) =>
      ...
      (specialtyFilter === '' || trainer.specialization === specialtyFilter)
  );

  // Get unique specializations for filter dropdown
  const specialties = [...new Set(trainers.map((trainer) => trainer.specialization).filter(Boolean))];
            ...
            <Col key={trainer._id} md={4} className="mb-4">
              <Card className="h-100 trainer-card">
                <div className="trainer-image-container">
                  <Card.Img
                    variant="top"
                    src={trainer.photo || 'https://images.unsplash.com/photo-1517836357463-d25dfeac3438?q=80&w=1200&auto=format&fit=crop'}
                    alt={trainer.name}
